...
Initial run: Aug 19, 2026 (Finished OK!)
"""
...
# ---- CONFIG ----------------------------------------------------------
COMMUNITY = "eol"
# OR match: record kept if ANY keyword matches
KEYWORDS = ["textmining", "traits"]
...
def fetch_community_records(community):
    ...
    while url:
        resp = requests.get(url, params=params if url ==
                            BASE_URL else None, headers=headers)
        if resp.status_code != 200:
            print(
                f"Error {resp.status_code} on page {page_num}: {resp.text[:300]}", file=sys.stderr)
            break
        ...
        print(
            f"  fetched page {page_num}: {len(hits)} records (running total: {len(all_hits)})")

        # None once there's no next page
...
def main():
    ...
    print(
        f"Records matching keywords {KEYWORDS} ({MATCH_MODE}): {len(filtered)}\n")
    ...
    for r in records:
        print(
            f"{r['total_size']:>15,}  {r['n_files']:>5}  {r['id']:>10}  {r['title'][:65]}")

    # ---- write CSV ----
    with open(OUTPUT_CSV, "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(["rank", "id", "doi", "total_size_bytes",
                        "n_files", "keywords", "title", "url"])

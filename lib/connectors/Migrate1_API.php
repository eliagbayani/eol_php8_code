...
class Migrate1_API
{
    ...
    */
    ...
    private function process_Vernacularname()
    { 
        echo "\nprocess_Vernacularname...\n"; $i = 0;
        $v = new \eol_schema\VernacularName();
        $v->taxonID = md5('Gadus morhua');
        $v->vernacularName = 'Atlantic cod';
        $v->language = "en";
    ...
    private function process_Reference()
    {   //print_r($meta);

        $r = new \eol_schema\Reference();
        $r->full_reference = "The quick brown fox jumps over the lazy dog.";
        $r->identifier = md5($r->full_reference);
        $r->uri = "http://eli.org/doing/good.html";        
        $this->archive_builder->write_object_to_file($r);

        $r = new \eol_schema\Reference();
        $r->full_reference = "The Empire Strikes Back";
        $r->identifier = md5($r->full_reference);
        $r->uri = "http://eli.org/doing/alright.html";        
        $this->archive_builder->write_object_to_file($r);

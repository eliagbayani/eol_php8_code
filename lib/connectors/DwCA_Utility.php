...
class DwCA_Utility
{
    ...
    {   /* param $preferred_rowtypes is the option to include-only those row_types you want on your final DwCA. 1st client was DATA-1770 */
        ...
        elseif(in_array($this->resource_id, array("gbif_classification", "gbif_classification_without_ancestry", "gbif_classification_final", 
                                                  ...
                                                  "wikipedia_en_traits_tmp4", "21_ENV_03", 
        ...
        */
        ...
        if(in_array($this->resource_id, array("wikipedia_en_traits_tmp1", "wikipedia_en_traits_tmp4", "21_ENV_03"))) { //calls a generic utility
            require_library('connectors/ResourceUtility');
            $func = new ResourceUtility($this->archive_builder, $this->resource_id);
        ...
        if(@$this->params['resource'] == "neo4j_prep") {
            require_library('connectors/DwCA_Parse_Taxa_and_MoF_API');
            $func = new DwCA_Parse_Taxa_and_MoF_API($this->archive_builder, $this->resource_id, $this->archive_path);
            $func->start($info);
        }
        ...
        if(@$this->params['resource'] == "use_EOLid_as_taxonID") {
            require_library('connectors/DwCA_UseEOLidInTaxa');
            $func = new DwCA_UseEOLidInTaxa($this->archive_builder, $this->resource_id, $this->archive_path);
            $func->start($info);
        }
        // -------------------- */
    ...
    function convert_archive_files($lifedesks) //used by: connectors/lifedesk_eol_export.php
    {
        ...
        foreach($lifedesks as $ld) //e.g. $ld = "LD_afrotropicalbirds" or "LD_afrotropicalbirds_multimedia"
        {
            ...
            // $dwca_file = WEB_ROOT."/applications/content_server/resources/".$ld.".tar.gz";
        ...
        foreach(new FileIterator($meta->file_uri) as $line => $row) {
            ...
            elseif($purpose == 'write') {
                ...
                if($class == "document") { //if object is text type then description cannot be blank.
                    if($o->type == 'http://purl.org/dc/dcmitype/Text') {
